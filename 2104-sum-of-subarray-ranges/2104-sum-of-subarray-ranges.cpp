...
void leftansm(vector<int>&arr,vector<int>&left,int n){
    stack<int>st;
    for(int i=0;i<n;i++){
       if(st.empty()){
            left[i]=-1;
...
void rightansm(vector<int>&arr,vector<int>&right,int n){
    stack<int>st;
    for(int i=n-1;i>=0;i--){
       if(st.empty()){
            right[i]=n;
    ...
    long long subArrayRanges(vector<int>& arr) {
        ...
        leftansm(arr,leftm,n);
        rightansm(arr,rightm,n);
         long long sum2=0;
   
        for(int i=0;i<n;i++){
            long long la=i-leftm[i];
            long long ra=rightm[i]-i;
            long long total=la*ra;
            long long totalsum=arr[i]*total;
            sum2=(sum2+totalsum);
        }
